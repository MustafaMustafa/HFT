...
//________________________________________________________________________________
void StAnalysisMaker::PrintPixelHits() {
  UInt_t i=0,k=0,l;
  //  Double_t zPrim = 0;
  StEvent* pEvent = (StEvent*) StMaker::GetChain()->GetInputDS("StEvent");
  if (!pEvent) return;
  ...
  StSPtrVecPxlHit &hits = PxlHitCollection->hits();
  UInt_t NoHits = hits.size();
  for (l = 0; l < NoHits; l++) {
    StPxlHit *hit = hits[l];
    if (hit) {
...
void StAnalysisMaker::summarizeEvent(StEvent *event, Int_t mEventCounter) {
  ...
  if (pxlhits) {
    StPxlHit *hit;
    const StSPtrVecPxlHit &hits = pxlhits->hits();
    //if (!pxlhits) { cout << "No Pxl Hit Collection" << endl; return;}
    Int_t NoHits =  pxlhits->numberOfHits();
    LOG_DEBUG  <<" size of pxl HitCollectiion :  "<< NoHits << endm;
    UInt_t         totalNoOfHits=0;
    UInt_t         noBadHits=0;
    UInt_t         noHitsUsedInFit=0;
    ...
    */           
    ...
    for(const_StPxlHitIterator it=hits.begin(); it!=hits.end(); ++it){
      ...
      //test  = (*hits)[i];
      //StPxlHit *hit = (StPxlHit *) test;
      //hit = hits[i];
      //Int_t j = 0;
      //for (j = 0; j < NHtypes-1; j++) if ( Hits[j].kId == hit->detector()) break;
      totalNoOfHits++;
      if (hit->flag())  noBadHits++;
      if (hit->usedInFit()) noHitsUsedInFit++;
    }
    LOG_QA << "# of PXL hits:         " << totalNoOfHits
	   << ":\tBad ones: " << noBadHits
	   << ":\tUsed in Fit:      " << noHitsUsedInFit << endm;
  }
  /*
