...
Int_t StPixelFastSimMaker::Make()
{
  ...
  StPxlHitCollection *col = new StPxlHitCollection;
  if (!col )
    {
  ...
  if (pixHitCol){
	...
	if (pixHitCol->layer(k))
	  ...
	  for (UInt_t i = 0; i < nh; i++){
	    ...
	    //StPRnDHit* tempHit = new StRnDHit(gpixpos, mRndHitError, 1, 1., 0, 1, 1, id++, kPxlId);
	    StPxlHit* tempHit = new StPxlHit(gpixpos, mRndHitError, 1, 1., 0, 1, 1, id++, kPxlId);
	    tempHit->setVolumeId(mcPix->volumeId());
	    tempHit->setSector(k+1);
...
LOG_DEBUG<<"pixel rnd hit location x: "<<tempHit->position().x()<<"; y: "<<tempHit->position().y()<<"; z: "<<tempHit->position().z()<<endm;
    ...
    gMessMgr->Info() <<"StPxlFastSimMaker::Make() -I- Loaded "<<nhits<<"pixel hits. \n";
  }
  else{
  ...
  */
  rcEvent->setPixelHitCollection(col);
  LOG_DEBUG <<" size of hit collection : " << col->numberOfHits() << endm;
  return kStOK;
    }
